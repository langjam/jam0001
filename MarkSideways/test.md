...
Most programming languages have code where you can add comments in the 
code to make the code clearer. Conversely, Marksideways uses markdown files
as the actual code document. Code snippets are added to the markdown and
their purpose and context are defined by the structure of the markdown file
itself, through the use of headings, sub-headings, and bulleted lists.
...
begins. For example, this is a simple Hello World line of code:
...
are also included within the programs initial execution code. You can think
...
how variables, methods, and loops work.
...
but unambiguous traditional syntax when instantiating a class.
...
Because this method uses no arguments, there is no bulleted list here. We use
the instance fields `begin` and `end` in a `for` loop to loop through these
numbers.

A for loop is constructed by using the word `for` followed by a variable name
followed by start and end points. The end point can be inclusive if you use
the word `thru` or it can be exclusive if you use the word `till`. In this
case we want to include the value in `this.end` in our counting, so we use `thru`.
