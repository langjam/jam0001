...
function main(args) {
    ...
    if (args.dump_ast) pprint_ast(parser_results.parsetree)
    const compiler_results = compile(parser_results.parsetree, parser_results.comments, args.filename)
    
    if (args.dump_bytecode) disassemble(compiler_results)
...
function parse_args() {
    ...
    for (const arg of process.argv.slice(2)) {
        if (arg === '--no-execute') flags.execute = false
        else if (arg === '--dump-ast') flags.dump_ast = true
        else if (arg === '--dump-bytecode') flags.dump_bytecode = true
        else flags.filename = arg
    }
